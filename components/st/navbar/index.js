      ...
      <div className={style.navbar}>
        ...
        <div className={style.navbar_group}>
          <div className={style.navbar_btn}>
            <Link onMouseEnter={() => handleMouseEnter("book")} href="/st/book">
              實體書籍
            </Link>
            <Link href="/st/forum">討論交流</Link>
            <Link href="/st/milu">關於作者</Link>
          </div>
          ...
          <div className={style.slide_list_btn}>
            {bookdata.map((v) => (
              <div key={v.id} className={style.slide_list_link}>
                <Link href={`/st/book?b_genre=${v.name}`}>
                  {v.b_genre}
                  {v.name}
                </Link>
              </div>
